...
export function createDinoGame(
  ...
  function drawDino(W: number, H: number, color: number) {
    ...
    dinoGfx
      .clear()
      .roundRect(DX - DINO_W / 2, topY, DINO_W, DINO_H, 6)
      .fill(color);
    if (!s.dead) {
      eyeGfx
        .clear()
        .circle(DX + 8, topY + 12, 4)
        .fill(0x08080f)
        .circle(DX + 9, topY + 11, 1.5)
        .fill(0xffffff);
    } else {
      eyeGfx.clear();
    }
  ...
  return {
    ...
    onKey(key: string) {
      if (key === controlKey && s.onGround && !s.dead) {
        s.vy = JUMP_VY;
        s.onGround = false;
      }
    },
    ...
    update(dt, W, H) {
      ...
      if (s.nextObstacleIn <= 0) {
        ...
        s.nextObstacleIn =
          Math.max(0.5, 1.5 - s.timePlayed / 60) + Math.random() * 0.8;
      }
      ...
      // Collision
      const dL = DX - DINO_W / 2 + 5,
        dR = DX + DINO_W / 2 - 5;
      const dB = GY - s.yOffset - 3;
      for (const obs of obstacles) {
        if (
          dR > obs.gfx.x + 3 &&
          dL < obs.gfx.x + obs.w - 3 &&
          dB > GY - obs.h + 2
        ) {
          s.dead = true;
          s.deathFlash = 18;
