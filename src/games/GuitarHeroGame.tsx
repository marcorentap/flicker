...
export function createGuitarHeroGame(
  ...
  function spawnHitParticle(W: number, H: number) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      (p as Graphics & { vx: number; vy: number }).vx = Math.cos(angle) * 80;
      (p as Graphics & { vx: number; vy: number }).vy = Math.sin(angle) * 80;
  ...
  function hitNote() {
    ...
    const note = s.notes.find(
      (n) =>
        !n.hit && !n.missed && n.y + NOTE_H >= hitZoneTop && n.y <= hitZoneBot,
    );
    if (note) {
      note.hit = true;
      note.hitY = note.y;
  ...
  return {
    ...
    update(dt, W, H) {
      ...
      if (s.nextNoteIn <= 0) {
        ...
        s.nextNoteIn =
          Math.max(0.7, 1.8 - s.timePlayed / 40) * (0.75 + Math.random() * 0.5);
      }
      ...
      for (const n of s.notes) {
        if (n.hit) {
          ...
          const inZone = n.y + NOTE_H >= hitZoneTop && n.y <= hitZoneBot;
          notesGfx
            .rect(trackX + 4, n.y, trackW - 8, NOTE_H)
            .fill(inZone ? 0xffffff : colorNum);
          notesGfx.rect(trackX + 4, n.y, trackW - 8, NOTE_H).stroke({
