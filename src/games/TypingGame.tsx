...
const _all: string[] = (allWords as string[]).filter((w: string) => /^[a-z]+$/.test(w));
...
export function createTypingGame(
    ...
    container.addChild(bg, beamGfx, wordsContainer, fxGfx, scoreText, gameOverBg, gameOverTitle, gameOverScore);
    ...
    function spawnWord(W: number) {
        ...
        const pool = difficulty < 0.3 ? WORDS_SHORT
            : difficulty < 0.6 ? WORDS_MEDIUM
            : WORDS_LONG;
        ...
        const typedText = new Text({
            text: '',
            style: { fontFamily: 'monospace', fontSize: FONT_SIZE, fill: colorNum, fontWeight: 'bold' },
        });
        ...
        words.push({ id: nextId++, word, x, y: -(FONT_SIZE + 4), speed, typedIdx: 0, locked: false, typedText, remainText });
    }
    ...
    function emitParticles(x: number, y: number, count: number) {
        for (let i = 0; i < count; i++) {
            ...
            particles.push({ x, y, vx: Math.cos(angle) * spd, vy: Math.sin(angle) * spd, life: 0.3 + Math.random() * 0.2, maxLife: 0.5 });
        }
    }
    ...
    return {
        ...
        onKey(key: string) {
            ...
            if (lockedWord) {
                ...
                if (k === expected) {
                    ...
                    const charX = lockedWord.x + lockedWord.typedIdx * CHAR_W + CHAR_W * 0.5;
                    ...
                    if (lockedWord.typedIdx >= lockedWord.word.length) {
                        ...
                        emitParticles(lockedWord.x + lockedWord.word.length * CHAR_W * 0.5, lockedWord.y, 10);
        ...
        update(dt, W, H) {
            ...
            bg.clear()
                .rect(0, 0, W, H).fill(0x08080f)
                .moveTo(0, H - 48).lineTo(W, H - 48)
            ...
            for (let i = beams.length - 1; i >= 0; i--) {
                ...
                const a = b.life / 0.12;
                beamGfx
                    .moveTo(b.x1, b.y1).lineTo(b.x2, b.y2)
            ...
            for (let i = words.length - 1; i >= 0; i--) {
                ...
                if (w.y > H + 10) {
                    ...
                    if (!gameOverFired) {
                        gameOverFired = true;
                        dead = true;
                        for (const rem of words) { rem.typedText.visible = false; rem.remainText.visible = false; }
                        gameOverBg.clear().rect(0, 0, W, H).fill({ color: 0x000000, alpha: 0.7 });
